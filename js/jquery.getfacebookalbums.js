 ...
 *	
 * Details on original version, in french, at http://www.davel.fr/techblog/2010/06/plugin-jquery-get-facebook-albums-photos
 * Date: 08/11/2012
...
(function ($) {
	...
	$.fn.getFacebookAlbums = function (pOptions) {		
		...
		if (options.appId == null) {
			$(this).html('The AppId is not set, try this code: ' +
                '$(\'#reference\').getFacebookAlbums({appId:\'YOUR-APP-ID\'})');
			return;
		}		
		...
		$(this).html('<div id="fbListAlbumsContainer"><a href="#">' + 
		  options.startConnectLabel + '</a></div><div id="fbImagesContainer">' +
	...
	login = function () {
		$('#fbListAlbumsContainer').html( options.albumsLoadingLabel + 
		    '<br><img src="' + options.loadingImage + '" />');
		FB.init({
			appId: options.appId,
			cookie: true,
			status: true
		...
		FB.login(function (response) {
		    //response.scope && response.scope.indexOf('user_photos') != -1) {
	...
	onFBAlbumSelected = function () {
		$('#fbImagesContainer').html(options.imagesLoadingLabel + 
            '<br><img src="' + options.loadingImage + '">');
	...
	onFBPhotoSelected = function () {
		...
		contentHTML = '<p><img src="' + photosData[$(this).attr('id').replace('image_','')].src + 
		              '"></p><div id="fbValidatePhoto">' + options.pickImageLabel + '</div>';
